...
- [ ] **Gaps, not silently glossed over**: (1) CasaOS manifest references `ghcr.io/raktim94/billing-platform-{server,worker}:0.1.0` images that don't exist yet — `.github/workflows/docker-publish.yml` (multi-arch, adapted from nodedr-pos's real one) is written but hasn't run against a real tag; not installable from a real CasaOS instance until it does. (2) Icon/thumbnail/screenshot assets are placeholder paths — no real design assets exist (no frontend/brand work has started). (3) Backup/restore automation is explicitly Stage 11 scope, not built here — `docs/operations/deployment.md` documents a manual `pg_dump` fallback and says so directly, doesn't imply automation exists. (4) Redis/minio profiles are provisioned but genuinely unwired (no code reads `REDIS_URL`; no file-storage abstraction exists yet).
...
## Stage 10b-2 — Frontend Feature Screens (not started)
- [ ] Sales screen — brief §48's "exceptional attention" screen: barcode/keyboard-driven billing, customer balance/credit-limit visibility, stock/price/tax visibility, hold/restore draft, print-after-finalize
- [ ] Inventory, purchases, contacts, catalogue, accounting, GST/tax, reports screens wired to their already-built and tested backend APIs
- [ ] Free-first e-Way Bill UX (backend built in Stage 8c, `docs/architecture.md` §9b): invoice-screen EWB card (Ready/Needs Information/Complete-on-Portal states), Settings → GST → E-Way Bill page (Free Government Portal selected by default, Automatic API as the alternate radio), the "portal assistant" post-click screen (prepared filename, open-portal button, popup-blocked fallback), a pending-tasks center for unfinished EWBs, a bulk-prepare queue, vehicle/transporter master screens with autocomplete, result-import UI (file/PDF/manual) with the mismatch-warning path, first-time-use explainer shown once — never expose "JSON"/"API"/technical terms in this UI, plain language only per §9b
- [ ] Global search and quick-create wired to real functionality (currently visual placeholders from Stage 10b-1)
- [ ] Accessibility pass (WCAG 2.2 AA) and code-splitting for bundle size, once there's more than one page's worth of code to split
